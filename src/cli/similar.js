...
const USAGE = '使い方: node src/cli/similar.js <id> [--limit N] [--json]';

function parseArgs(argv) {
  const { flags, positional } = parseFlags(argv, SPEC);
  return { limit: 10, json: false, ...flags, id: Number(positional[0]) };
}
...
function main() {
  ...
  try {
    ...
    if (args.json) {
      console.log(JSON.stringify(enriched.map((r) => ({ id: r.book.id, title: r.book.title, score: r.score })), null, 2));
    } else {
